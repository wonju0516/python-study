...
venv\Scripts\activate            # * 활성화 명령어만 다름 (source venv/bin/activate 아님)
pip install -r requirements.txt
```

- Windows에서 `venv\Scripts\activate` 실행 시 "이 시스템에서 스크립트 실행이 금지되어 있습니다" 같은 오류가 나면, PowerShell을 관리자 권한으로 열어서 `Set-ExecutionPolicy RemoteSigned` 한 번 실행해줘야 함
