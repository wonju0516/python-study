...
class Node: # * 클래스는 값과 그 값을 다루는 함수(메서드)까지 같이 묶을 수 있는 것
    def __init__(self, data, next=None): # * 이 클래스에 있는 함수
        ...
        self.next = next  # ? 다음 노드를 가리키는 포인터 (기본값 None -> 아직 다음 노드 없음)
...
class LinkedList: # * 그 노드들을 서로 연결해서 관리하는 전체 리스트에 대한 설계도
    ...
    def append(self, data): 
        ...
        while current.next: # * current.next가 None이 아닌 동안 계속 반복 -> 리스트 마지막까지 이동
            current = current.next 
        current.next = Node(data) # * current가 리스트 마지막 요소로 이동했으니 그 next에 넣기
    ...
    def remove(self, target):
        ...
        while current: # * 링크드 리스트 순환중
            ...
            previous = current # ? 못 찾았으니 한 칸 이동: previous를 지금 current 위치로
...
for item in d:  # ? print(d) 대신 굳이 for문 쓴 이유: 값을 한 줄씩 따로 출력하려고 (없어도 됨)
    print(item)
...
for i in range(0, 20): # * 0~19 -> 20개의 숫자
