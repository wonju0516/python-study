...
print(
    c
)  # * Counter({2: 7, 1: 5, 3: 5}) -> {값: 등장횟수} 형태. 삽입 순서가 아니라 등장 횟수 내림차순으로 보여줌
print(list(c))  # * list()로 감싸면 값(key)들만 뽑힘 -> [1, 2, 3]

c = Counter("Hello")
print(c)  # * 문자열도 한 글자씩 세어줌. (대소문자 구분)

sentences = "hello hello hello a wonderful world"
c = Counter(
    sentences.split()
)  # * split()으로 먼저 단어 리스트로 쪼갠 뒤 Counter에 넣음 -> 단어별 등장 횟수
print(c)
...
dd[
    "there"
]  # * 아직 없는 key를 그냥 "읽기"만 해도, 그 순간 자동으로 dd["there"] = 0 이 생성됨
...
print(
    t[1]
)  # * 일반 튜플은 인덱스로만 접근 가능해서, 뭐가 뭔지(나이인지 몸무게인지) 이름만 봐선 알 수 없음

# * namedtuple("타입이름", [필드명, ...]): 이 필드들을 가진 새로운 튜플 "타입"을 만들어줌
Dog = namedtuple("Dog", ["age", "breed", "name"])
print(
    Dog
)  # * <class '__main__.Dog'> -> Dog는 값이 아니라 새로 정의된 클래스(타입) 자체

sammy = Dog(
    age=5, breed="Husky", name="Sammy"
)  # * 이 타입으로 실제 인스턴스(튜플) 생성
